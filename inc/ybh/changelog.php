...
function ybh_changelog_entries()
{
    $entries = array(
        ...
        array(
            'version' => '1.3.29',
            'date'    => '2026-09-26',
            'title'   => '展台文字三段式 · 封面改填充 · 引入真衬线字体',
            'items'   => array(
                array('type' => 'feat', 'scope' => '首页', 'text' => '**展台卡片文字改为三段式**：① 主标题（衬线粗体，居中偏上）、② 副标题（衬线细体，紧跟标题下方、字号更小）、③ 说明（维持原样）。后台展台项原本只有「标题」一个字段，现在**按标题里的分隔符 `|` 自动拆分**——例如「小说|另辟星河」显示为大字「小说」＋小字「另辟星河」。现网 12 项里有 7 项是这种写法，**无需改任何数据**；没有分隔符的（如「日志通知」）只显示主标题。'),
                array('type' => 'fix', 'scope' => '首页', 'text' => '**展台封面改为填充铺满**：此前站点开着「展台图片完整显示（防裁切）」，图片比例与卡片不一致时两侧留白、靠「同一张图放大模糊」补底；现在改为填充，不留白、也不再有那层模糊底。**那个开关仍然有效**——它照旧管着文章列表卡片的封面，只是展台这一路按你的要求固定为填充。'),
                array('type' => 'feat', 'scope' => '排版', 'text' => '**引入真正的衬线字体**（Noto Serif SC，SIL OFL 1.1），自托管在本站、不再依赖 Google：此前标题字体栈里虽然写着这个族名，但服务器上没有该字体，标题实际落在手写体上靠浏览器**合成加粗**；现在标题是真衬线粗体、副标题是真衬线细体（合成做不出可信的细体）。同时**修好**了之前一条特指度极高的语言字体规则一直夺走标题字体的问题。'),
                array('type' => 'perf', 'scope' => '性能', 'text' => '**首屏字体没有因此变重**：衬线体按字符用法分片，并把「站点标题实际用到的 838 个字」单独切了一个小面（声明在分片之后并**把码位从分片里减掉**，两者零重叠 ⇒ 命中确定、不重复下载）。实测首页**一个分片都不拉**，只下 140 KB 的子集（若不这么做会把 20 个分片、约 1.4 MB 一起拉下来）。'),
                array('type' => 'feat', 'scope' => '排版', 'text' => '**标题字号整体略增**：首页「展台 / 文章」章节标题 18 → 20 px、分类与标签页标题 30 → 32 px、文章页标题 24 → 26 px；封面风格页的 40 px 大标题**保持原样**。**文章卡片里的字号一格未动**（仍为 17 px / 窄屏 13.5 px），只是展台卡片内的主标题由 13.6 px 提到 16.8 px。'),
            ),
        ),
        ...
        array(
            'version' => '1.3.28',
            'date'    => '2026-09-25',
            'title'   => '修「打一个换行、发布后变两个」—— 服务端段落规则与 WordPress 对齐',
            'items'   => array(
                array('type' => 'fix', 'scope' => '编辑器', 'text' => '**修掉「正文里敲一个换行，发布后变成两个」**：上一版（1.3.25）为了让编辑器里敲的空行不被吃掉，加了一道服务端兜底；但那条兜底用的是「编辑器的存储格式」，与 WordPress 自己在保存时写进 textarea 的格式**不是同一套** —— 于是 WordPress 用来分隔段落的换行被当成了作者敲的空行，**每一段后面都被塞进一个空行**，而且**每保存一次就再翻一倍**（3 段 → 5 → 9 → 17…）。现在服务端改按 WordPress 自己的段落模型解析：空行才是段落边界、段落内部的单个换行保持原样。实测 8/8 用例通过，连续保存 4 次段数稳定不变。'),
                array('type' => 'fix', 'scope' => '编辑器', 'text' => '**编辑器里敲的空行依然保留**（1.3.25 的那条诉求没有被这次修复推翻）：作者手动敲的空行照样存成空行、发布后照样显示。两条规则的分工是——**保存时**服从 WordPress 的存储格式，**粘贴时**才按「1 个回车 = 分段、2 个回车 = 空行」处理你从别处复制来的文档。'),
                array('type' => 'fix', 'scope' => '内容', 'text' => '**修复此前已被写脏的文章**：把被多插出来的空行清理掉，正文恢复正常段距。改动前已整表备份（`wp_posts_bak_T54`），可整体回滚。'),
            ),
        ),
